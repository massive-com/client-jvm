
# GetGroupedCryptoAggregates200ResponseAllOfResultsInner

## Properties
| Name | Type | Description | Notes |
| ------------ | ------------- | ------------- | ------------- |
| **T** | **kotlin.String** | The exchange symbol that this item is traded under. |  |
| **c** | **kotlin.Double** | The close price for the symbol in the given time period. |  |
| **h** | **kotlin.Double** | The highest price for the symbol in the given time period. |  |
| **l** | **kotlin.Double** | The lowest price for the symbol in the given time period. |  |
| **o** | **kotlin.Double** | The open price for the symbol in the given time period. |  |
| **t** | **kotlin.Int** | The Unix millisecond timestamp for the end of the aggregate window. |  |
| **v** | **kotlin.Double** | The trading volume of the symbol in the given time period. |  |
| **n** | **kotlin.Int** | The number of transactions in the aggregate window. |  [optional] |
| **vw** | **kotlin.Double** | The volume weighted average price. |  [optional] |



