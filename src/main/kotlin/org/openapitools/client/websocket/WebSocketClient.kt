...
import io.massive.kotlin.sdk.DefaultJvmHttpClientProvider
import io.massive.kotlin.sdk.HttpClientProvider
import io.massive.kotlin.sdk.Version
import io.massive.kotlin.sdk.ext.MassiveCompletionCallback
import io.massive.kotlin.sdk.ext.coroutineToCompletionCallback
import io.massive.kotlin.sdk.websocket.MassiveWebSocketMessage.*
...
sealed class Feed(val url: String) {
    object Delayed : Feed("delayed.massive.com")
    object RealTime : Feed("socket.massive.com")
    object Nasdaq : Feed("nasdaqfeed.massive.com")
    object PolyFeed : Feed("polyfeed.massive.com")
    object PolyFeedPlus : Feed("polyfeedplus.massive.com")
    object StarterFeed : Feed("starterfeed.massive.com")
    object LaunchpadFeed : Feed("launchpad.massive.com")
    object BusinessFeed : Feed("business.massive.com")
    object EdgxBusinessFeed : Feed("edgx-business.massive.com")
    object DelayedBusinessFeed : Feed("delayed-business.massive.com")
    object DelayedEdgxBusinessFeed : Feed("delayed-edgx-business.massive.com")
    object DelayedNasdaqLastSaleBusinessFeed : Feed("delayed-nasdaq-last-sale-business.massive.com")
    object DelayedNasdaqBasicFeed : Feed("delayed-nasdaq-basic-business.massive.com")
    object DelayedFullMarketBusinessFeed : Feed("delayed-fullmarket-business.massive.com")
    object FullMarketBusinessFeed : Feed("fullmarket-business.massive.com")
    object NasdaqLastSaleBusinessFeed : Feed("nasdaq-last-sale-business.massive.com")
    object NasdaqBasicBusinessFeed : Feed("nasdaq-basic-business.massive.com")
...
/**
 * A client for the Massive.com WebSocket API
 *
 ...
 * @param listener the [MassiveWebSocketListener] to send events to
 ...
 * @param massiveWebSocketDomain (Optional) The domain to connect to for all websockets; defaults to
 * Massive's websocket domain "socket.polyhon.io". Useful for overriding in a testing environment
...
constructor(
        ...
        private val listener: MassiveWebSocketListener,
    ...
    /**
     * Connect and authenticate to the given [MassiveWebSocketCluster].
     *
    ...
    suspend fun connect() {
        ...
        try {
            session.incoming
                    ...
                    .onCompletion { listener.onDisconnect(this@MassiveWebSocketClient) }
    ...
    fun connectAsync(callback: MassiveCompletionCallback?) =
     ...
     */
    suspend fun subscribe(subscriptions: List<MassiveWebSocketSubscription>) {
    ...
    fun subscribeBlocking(subscriptions: List<MassiveWebSocketSubscription>) = runBlocking {
        subscribe(subscriptions)
    }

    /** Async/callback version of [subscribe] */
    fun subscribeAsync(
            subscriptions: List<MassiveWebSocketSubscription>,
     ...
     */
    suspend fun unsubscribe(subscriptions: List<MassiveWebSocketSubscription>) {
        if (subscriptions.isEmpty()) return
    ...
    fun unsubscribeBlocking(subscriptions: List<MassiveWebSocketSubscription>) = runBlocking {
        unsubscribe(subscriptions)
    }

    /** Callback/async version of [unsubscribe] */
    fun unsubscribeAsync(
            subsciptions: List<MassiveWebSocketSubscription>,
            callback: MassiveCompletionCallback?
    ...
    fun disconnectAsync(callback: MassiveCompletionCallback?) =
    ...
    private fun processFrameJson(
            ...
            feed: Feed,
            frame: JsonElement,
            collector: MutableList<MassiveWebSocketMessage> = mutableListOf()
    ): List<MassiveWebSocketMessage> {
    ...
    private fun parseStockMessage(frame: JsonObject): MassiveWebSocketMessage {
    ...
    private fun parseOptionMessage(frame: JsonObject): MassiveWebSocketMessage {
        return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
            "status" -> serializer.decodeFromJsonElement(StatusMessage.serializer(), frame)
    ...
    private fun parseIndicesMessage(frame: JsonObject): MassiveWebSocketMessage {
        return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
            "status" -> serializer.decodeFromJsonElement(StatusMessage.serializer(), frame)
    ...
    private fun parseForexMessage(frame: JsonObject): MassiveWebSocketMessage {
        return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
            "status" -> serializer.decodeFromJsonElement(StatusMessage.serializer(), frame)
    ...
    private fun parseCryptoMessage(frame: JsonObject): MassiveWebSocketMessage {
        return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
            "status" -> serializer.decodeFromJsonElement(StatusMessage.serializer(), frame)
	...
	private fun parseFuturesMessage(frame: JsonObject): MassiveWebSocketMessage {
	    return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
    ...
    private fun parseLaunchpadMessage(frame: JsonObject): MassiveWebSocketMessage {
        return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
            "status" -> serializer.decodeFromJsonElement(StatusMessage.serializer(), frame)
            "LV" ->
                    serializer.decodeFromJsonElement(
    ...
    private fun parseBusinessMessage(frame: JsonObject): MassiveWebSocketMessage {
        return when (frame.jsonObject[EVENT_TYPE_MESSAGE_KEY]?.jsonPrimitive?.content) {
            "status" -> serializer.decodeFromJsonElement(StatusMessage.serializer(), frame)
            "FMV" ->
                    serializer.decodeFromJsonElement(
    ...
    private fun parseOtherMessage(frame: JsonObject): MassiveWebSocketMessage {
