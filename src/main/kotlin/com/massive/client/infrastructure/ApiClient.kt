package com.massive.client.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val BASE_URL_KEY: String = "com.massive.client.baseUrl"
        @Deprecated(
        ...
        if (requestConfig.query["apiKey"].isNullOrEmpty()) {
            if (apiKey["apiKey"] != null) {
                if (apiKeyPrefix["apiKey"] != null) {
                    requestConfig.query["apiKey"] = listOf(apiKeyPrefix["apiKey"]!! + " " + apiKey["apiKey"]!!)
                } else {
                    requestConfig.query["apiKey"] = listOf(apiKey["apiKey"]!!)
                }
            }
        }
    }
